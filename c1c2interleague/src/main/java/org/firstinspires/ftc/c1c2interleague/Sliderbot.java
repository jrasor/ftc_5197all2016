 ...
 * JMR v 0.3 2/2/17 Camera back to port side.
 ...
 *   initUnEncodedDrive. Tuned goToBeacon with variable approach speed.
 *
 */
...
// ** This needs to be in a separate, reusable package.
    private static final double SQRT2 = Math.sqrt(2.0);
    private static final double COUNTS_PER_MOTOR_REV = 1120.0; // 1440 for TETRIX Encoder
    private static final int MAX_COUNTRATE = 2987; // max encoder rate for NeveRest 40
    private static final double DRIVE_GEAR_REDUCTION = 1.0;
    private static final double WHEEL_DIAMETER_INCHES = 3.96;   // calibrated
    private static final double DRIVEWHEEL_SEPARATION = 19.4;  // was 19.2
    ...
    /* Define and initialize the 4 motors. */
    private void idMotors(HardwareMap ahwMap) {
        hwMap = ahwMap;
    ...
    private void setMotorsDirections () {
    ...
    public void initUnencodedDrive(HardwareMap ahwMap) {
        ...
        // Generally, Autonomous modes will run
        // encoded, and Driver (TeleOp) modes
        //   will run unencoded.
    ...
    public void initEncodedDrive(HardwareMap ahwMap) {
        ...
        idMotors(ahwMap);
        setMotorsDirections();
        initMotorsRunMode(DcMotor.RunMode.RUN_USING_ENCODER);
    }
    ...
    //  ** To do: initSensors() ... so far, only the color sensor.
    ...
    */
    ...
    //  Command layer. Human driver issues commands with gamepad. Mecanum wheel drive.
    //  Not tempered.
    ...
    */
    ...
    //   Another simple wrapper. Just go sideways a number of inches. Slips
    //     irregularly fore and aft.
    public void driveSideways (double speed, double inches) {
        encoderDrive (speed, speed, speed, speed, inches, -inches, -inches, inches);
    }

    //   Wheel slippage can cause fore or aft drift here, too.
    public void turnAngle (double speed, double angle) {
        double inches = angle * DRIVEWHEEL_SEPARATION/SQRT2;
        encoderDrive (speed, speed, speed, speed, -inches, inches, -inches, inches);
    }

    /*  Turning movements. All angles are in radians.  ** Convert to mecanum build. */
    ...
    // ** make this a wrapper for encoderDrive instead. ** Convert to mecanum build.
    ...
    /* Go to a beacon, guided by Vuforia. Stop when camera is close enough. */
