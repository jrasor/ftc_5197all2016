 ...
 *  JMR 2/11/17 v. 0.5.1 Blue analog of Sliderbot RED BEACONS AUTO.
 */

@Autonomous(name="Sliderbot BLUE BEACONS AUTO", group="Interleague")
//@Disabled
public class SliderbotBlueAuto extends LinearOpMode {
    ...
    private final double TIGHT_TURN_RATIO = 0.40; // Was 0.40
    ...
    // Give time for pusher to hold beacon button, so beacon changes color.
    private final int BUTTON_HOLD_TIME = 750; // ms
    ...
    public void jobRunScript(){
        ...
        // ... Turn right to approximately face beacon image Wheels...
        robot.turnRight (.95, TIGHT_TURN_RATIO, 70.0);
        // ... go into it with robot vision guidance...

        robot.initMotorsRunMode(DcMotor.RunMode.RUN_USING_ENCODER);
        robot.goToBeacon();
        ...
        robot.driveStraight(0.5, -4.5);
        robot.driveSideways(0.5, 1.3); // ** This invites trouble.
        ...
        while(!seeColor) {
            ...
            telemetry.update();
            if ((colorCcache[0] & 0xFF) == 3)  // 10 for RED, 3 for Blue
            {
                robot.buttonPush.setPosition(0);
                seeColor = true;
                break;
            } else {
                robot.driveStraight(0.5, 1.0);
            }
        }
        sleep(BUTTON_HOLD_TIME); // This WORKS very well.
        robot.buttonPush.setPosition(1);

        // Now we get in position to observe second beacon image, Legos.
        robot.turnLeft(.95, WIDE_TURN_RATIO, 68.0);
        robot.turnAngle(Sliderbot.TURN_SPEED, -3.2*Math.PI/4);
        sleep(IMAGE_ID_TIME);

        robot.initMotorsRunMode(DcMotor.RunMode.RUN_USING_ENCODER);
        robot.goToBeacon();
        // ... turn 90 degrees right to present sensors and pusher to the
        // beacon buttons...
        robot.turnAngle(0.75, Math.PI/2);
        // ... Finally, back up and move in to present sensor to right half of the beacon.
        //   We will search from there with the color sensor.
        //robot.turnAngle(Sliderbot.TURN_SPEED, -Math.PI / 48);
        robot.driveStraight(0.5, -3.5); // was /4.0.
        robot.driveSideways(0.5, 1.3); // ** This invites trouble.
        // ** We can eliminate it with a different target X.
